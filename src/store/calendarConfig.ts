...
import { DEFAULT_SECOND_CAL_LOC, DEFAULT_MAIN_CAL_LOC, async_savePartialAppConfig } from '@/lib/appConfigDb';
...
export const useCalendarConfig = create<CalendarConfig>((set, get) => ({
	...
	setMainCal: (cl, save = true) => {
		set({ mainCal: cl });
		if (save) async_savePartialAppConfig({ mainCalendar: cl });
	},
	...
	setSecondCal: (cl, save = true) => {
		set({ secondCal: cl });
		if (save) async_savePartialAppConfig({ secondCalendar: cl });
	},
	...
	setSecondCalEnabled: (en, save = true) => {
		...
		if (save) async_savePartialAppConfig({ secondCalendarEnabled: en });
	}
}));
